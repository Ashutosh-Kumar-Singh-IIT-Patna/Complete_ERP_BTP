...
$data = [];
$formName = 'PhD Enrollment Program';  //Important
...
$sql = 'SELECT name_of_scholar, dept, roll, nationality, gender, scholar_phd_category, mobile, email, supervisor, co_supervisor, sponsored_agency_name, proj_num, proj_name, proj_tenure, broad_research_area, form_submit_flag_enrollment,
...
if ($result['success'] && $result['data']) {
    $data = $result['data'][0];
    $formSubmitted = ($data['form_submit_flag_enrollment'] == 1);  //Important
} else {
...
$dateOfSeminar = formatDate($data['date_of_enrollment'] ?? '');  //Important
...
$duration = calculateDuration($doctoralComitteFormation, $finalRecomendationForDegree);
    ...
    $pdf->Ln(10);
    $html = '<b>The Doctoral Committee recommends the candidates to enroll in the Ph.D program.</b>';
    $pdf -> MultiCell($available_width, 0,   $html,   0,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
