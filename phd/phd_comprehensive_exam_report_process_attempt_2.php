...
$data = [];
$formName = 'Comprehensive Exam Report (Attempt 2)';
...
if ($roll) {
    ...
    $sql = "SELECT name_of_scholar, dept, roll, nationality, gender, category, scholar_phd_category, mobile, email, proj_num, proj_name, proj_tenure, sponsored_agency_name, 
            form_submit_flag_compre_attempt_2, dc_chair, supervisor, co_supervisor, dc_internal_member, dc_external_member, dc_additional_member_1, dc_phd_coordinator,
    ...
    if ($result['success'] && $result['data']) {
        $data = $result['data'][0];
        $formSubmitted = ($data['form_submit_flag_compre_attempt_2'] == 1); // Check if form was already submitted
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $updateData = [];
    if (!empty($_POST['date_of_compre_attempt_2'])) {
        $updateData['date_of_compre_attempt_2'] = clean_input($_POST['date_of_compre_attempt_2']);
    }

    if (!empty($_POST['result_of_compre_attempt_2'])) {
        $updateData['result_of_compre_attempt_2'] = clean_input($_POST['result_of_compre_attempt_2']);
    }

    if (!empty($_POST['mode_of_compre_attempt_2'])) {
        $updateData['mode_of_compre_attempt_2'] = clean_input($_POST['mode_of_compre_attempt_2']);
    }

    if (!empty($updateData)) {
        $updateData['form_submit_flag_compre_attempt_2'] = 1;
        ...
        <div class="card shadow p-4">
            ...
            <?php else: ?>
                <form action="" method="POST">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold" for="date_of_compre_attempt_2">Date of <?php echo $formName ?>:</label>
                        <input type="text" id="date_of_compre_attempt_2" name="date_of_compre_attempt_2" class="form-control" placeholder="YYYY-MM-DD" required>
                        <span class="error"><?php echo $errors['date_of_compre_attempt_2'] ?? ''; ?></span>
                    </div>

                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold"> Result of <?php echo $formName ?>:</label>
                            <select name="result_of_compre_attempt_2" class="form-select" required>
                                <option value="Pass">Pass</option>
                                <option value="Fail">Fail</option>
                            </select>
                            <span class="error"><?php echo $errors['result_of_compre_attempt_2'] ?? ''; ?></span>
                        </div>

                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold"> Mode of Exam:</label>
                            <select name="mode_of_compre_attempt_2" class="form-select" required>
                                <option value="BOTH">BOTH</option>
                                <option value="ORAL">ORAL</option>
                                <option value="WRITTEN">WRITTEN</option>
                            </select>
                            <span class="error"><?php echo $errors['mode_of_compre_attempt_2'] ?? ''; ?></span>
                        </div>
                    </div>
                    ...
                    <script>
                        $(document).ready(function () {
                            $("#date_of_compre_attempt_2").datepicker({
