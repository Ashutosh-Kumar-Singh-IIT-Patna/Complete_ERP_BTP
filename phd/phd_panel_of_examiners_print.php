...
$sql = 'SELECT name_of_scholar, dept, roll, nationality, gender, scholar_phd_category, mobile, email, supervisor, co_supervisor, sponsored_agency_name, proj_num, proj_name, proj_tenure, broad_research_area, thesis_title, form_submit_flag_panel_of_examiners, panel_of_examiners,
...
if ($result['success'] && $result['data']) {
    $data = $result['data'][0];
    $formSubmitted = ($data['form_submit_flag_panel_of_examiners'] == 1);
} else {
...
$titleOfForm = $data['thesis_title'] ?? '';
$dateOfSeminar = formatDate($data['date_of_panel_of_examiners'] ?? '');
$examiners = json_decode($data['panel_of_examiners'] ?? [], true);
// $commentByDC = "Good";
...
$duration = calculateDuration($doctoralComitteFormation, $finalRecomendationForDegree);
    ...
    $pdf->Ln(3);
    $pdf->SetFont('times', '', 9);
    $html = '<b>Indian Examiners : </b>'.$projectName;
    $pdf -> MultiCell($available_width  , 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '<b>Name</b>'.$projectName;
    $pdf -> MultiCell(0.15 * $available_width  , 5,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '<b>Designation</b>'.$projectName;
    $pdf -> MultiCell(0.15 * $available_width  , 5,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '<b>Office Address</b>'.$projectName;
    $pdf -> MultiCell(0.15 * $available_width  , 5,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '<b>Home Page</b>'.$projectName;
    $pdf -> MultiCell(0.15 * $available_width  , 5,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '<b>Specialization</b>'.$projectName;
    $pdf -> MultiCell(0.15 * $available_width  , 5,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '<b>Email</b>'.$projectName;
    $pdf -> MultiCell(0.15 * $available_width  ,  5,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '<b>Telephone</b>'.$projectName;
    $pdf -> MultiCell(0.1 * $available_width  ,  5,   $html,   1,   'C',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);

    foreach($examiners['indian'] as $examiner) {
        $html = $examiner['name'];
        $pdf -> MultiCell(0.15 * $available_width  , 14,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
        $html = $examiner['designation'];
        $pdf -> MultiCell(0.15 * $available_width  , 14,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
        $html = $examiner['office_address'];
        $pdf -> MultiCell(0.15 * $available_width  , 14,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
        $html = $examiner['home_page'];
        $pdf -> MultiCell(0.15 * $available_width  , 14,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
        $html = $examiner['specialization'];
        $pdf -> MultiCell(0.15 * $available_width  , 14,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
        $html = $examiner['email'];
        $pdf -> MultiCell(0.15 * $available_width  , 14,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
        $html = $examiner['telephone'];
        $pdf -> MultiCell(0.1 * $available_width  ,  14,   $html,   1,   'C',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    
    }
    // $pdf->AddPage();

    $pdf->Ln(3);
    $html = '<b>Foreign Examiners : </b>'.$projectName;
    $pdf -> MultiCell($available_width  , 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '<b>Name</b>'.$projectName;
    $pdf -> MultiCell(0.15 * $available_width  , 5,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '<b>Designation</b>'.$projectName;
    ...
    foreach($examiners['foreign'] as $examiner) {
        $html = $examiner['name'];
        $pdf -> MultiCell(0.15 * $available_width  , 14,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
        $html = $examiner['designation'];
        $pdf -> MultiCell(0.15 * $available_width  , 14,   $html,   1,   'C',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
