...
$data = [];
$formName = 'Registration Seminar';  //Important
...
$sql = 'SELECT name_of_scholar, dept, roll, nationality, gender, scholar_phd_category, mobile, email, supervisor, co_supervisor, sponsored_agency_name, proj_num, proj_name, proj_tenure, broad_research_area, title_of_reg_1, result_of_reg_1, dc_comment_in_reg_1, form_submit_flag_reg_1,
...
if ($result['success'] && $result['data']) {
    $data = $result['data'][0];
    $formSubmitted = ($data['form_submit_flag_reg_1'] == 1);  //Important
} else {
...
$titleOfForm = $data['title_of_reg_1'] ?? '';  //Important
$dateOfSeminar = formatDate($data['date_of_reg_1'] ?? '');  //Important
$commentByDC = $data['dc_comment_in_reg_1'] ?? '';  //Important
$performance = $data['result_of_reg_1'];  // Important
