...
$sql = 'SELECT name_of_scholar, dept, roll, nationality, gender, scholar_phd_category, mobile, email, supervisor, co_supervisor, sponsored_agency_name, proj_num, proj_name, proj_tenure, broad_research_area, title_of_aps_5, result_of_aps_5, dc_comment_in_aps_5, form_submit_flag_aps_5,
...
if ($result['success'] && $result['data']) {
    $data = $result['data'][0];
    $formSubmitted = ($data['form_submit_flag_aps_5'] == 1);  //Important
} else {
...
$titleOfForm = $data['title_of_aps_5'] ?? '';  //Important
$dateOfSeminar = formatDate($data['date_of_aps_5'] ?? '');  //Important
$commentByDC = $data['dc_comment_in_aps_5'] ?? '';  //Important
$performance = $data['result_of_aps_5'];  // Important
