...
$data = [];
$formName = 'Comprehensive Exam Report (Attempt 2)';  //Important
...
$sql = 'SELECT name_of_scholar, dept, roll, nationality, gender, scholar_phd_category, mobile, email, supervisor, co_supervisor, sponsored_agency_name, proj_num, proj_name, proj_tenure, broad_research_area, form_submit_flag_compre_attempt_2, result_of_compre_attempt_2, mode_of_compre_attempt_2,
...
if ($result['success'] && $result['data']) {
    $data = $result['data'][0];
    $formSubmitted = ($data['form_submit_flag_compre_attempt_2'] == 1);  //Important
} else {
...
$dateOfSeminar = formatDate($data['date_of_compre_attempt_2'] ?? '');  //Important
$mode = $data['mode_of_compre_attempt_2'] ?? '';  //Important
$performance = $data['result_of_compre_attempt_2'] ?? '';  //Important
