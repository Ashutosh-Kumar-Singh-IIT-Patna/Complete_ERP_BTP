...
$data = [];
$formName = 'Final Recommendation for Viva-Voce';  //Important
...
$sql = 'SELECT name_of_scholar, dept, roll, nationality, gender, scholar_phd_category, mobile, email, supervisor, co_supervisor, sponsored_agency_name, proj_num, proj_name, proj_tenure, broad_research_area, thesis_title, form_submit_flag_viva_voce, dc_comment_summary_viva_voce, name_of_external_examiner, 
...
if ($result['success'] && $result['data']) {
    $data = $result['data'][0];
    $formSubmitted = ($data['form_submit_flag_viva_voce'] == 1);  //Important
} else {
...
$dateOfSeminar = formatDate($data['date_of_viva_voce'] ?? '');  //Important
$externalExaminer = $data['name_of_external_examiner'] ?? '';   
$commentByDC = $data['dc_comment_summary_viva_voce'] ?? '';  //Important
...
$duration = calculateDuration($doctoralComitteFormation, $finalRecomendationForDegree);
    ...
    // $pdf -> MultiCell($available_width, 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $pdf->Ln(10);
    $html = '<b>Recommendation of the DC:</b>';
    $pdf -> MultiCell($available_width , 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
