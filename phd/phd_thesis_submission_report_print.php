...
$data = [];
$formName = 'Thesis Submission Report';
...
$sql = 'SELECT name_of_scholar, dept, roll, nationality, gender, scholar_phd_category, mobile, email, supervisor, co_supervisor, sponsored_agency_name, proj_num, proj_name, proj_tenure, broad_research_area, thesis_title, form_submit_flag_thesis, 
...
if ($result['success'] && $result['data']) {
    $data = $result['data'][0];
    $formSubmitted = ($data['form_submit_flag_thesis'] == 1);
} else {
...
$dateOfSeminar = formatDate($data['date_of_thesis_submission'] ?? '');
// $commentByDC = "Good";
...
$duration = calculateDuration($doctoralComitteFormation, $finalRecomendationForDegree);
    ...
    $pdf->Ln(2);
    $html = '<b>The Scholar has submitted FOUR soft bound copies of the Thesis that are sent herewith for processing at your end.</b>';
    $pdf -> MultiCell($available_width , 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);

    $html = '<b>1. Clearance Certificate</b>';
    $pdf -> MultiCell(0.5 * $available_width, 0,   $html,   1,   'L',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    
    $html = '<b>2. Money Receipt of the thesis examination</b>';
    $pdf -> MultiCell(0.5 * $available_width, 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    
    $html = '<b>3. A Copy of the Certificate by the supervisor</b>';
    $pdf -> MultiCell(0.5 * $available_width, 0,   $html,   1,   'L',   false,   0,    '',    '',  true,  0,   true,  true,   0,   'T', false);

    $html = '<b>4. A Copy of the Certificate by the scholar</b>';
    $pdf -> MultiCell(0.5 * $available_width, 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);

    $pdf->Ln(2.5);
    $html = '<b>Signature of the HOD / HOC / HOS / Department Coordinator ________________</b>';
    $pdf -> MultiCell($available_width , 0,   $html,   0,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);

    $pdf->Ln(2);
    $html = '<b>To</b>';
    $pdf -> MultiCell($available_width , 0,   $html,   0,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '<b>The Assistant Registrar (Acadmeic)   IIT Patna</b>';
    $pdf -> MultiCell($available_width , 0,   $html,   0,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '<b>Encl: as Above</b>';
    $pdf -> MultiCell($available_width , 0,   $html,   0,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '<b> A copy of the Thesis Submission Form in respect of the research scholar ' . $name . ' is forwarded for information and necessary action at your end;</b>';
    $pdf -> MultiCell($available_width , 0 ,  $html,   0,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    
    $pdf->Ln(2.5);
    $html = '<b>NOTE: The Scholar will submit adequate number of hard-bound copies of the thesis before the viva-voce examination.
(One for each supervisor, One for Central Library and One for Department Library)</b>';
    $pdf -> MultiCell($available_width , 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
