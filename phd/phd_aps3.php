...
if ($roll) {
    ...
    $sql = "SELECT name_of_scholar, dept, roll, nationality, gender, category, scholar_phd_category, mobile, email, proj_num, proj_name, proj_tenure, sponsored_agency_name, form_submit_flag_aps_3
    ...
    if ($result['success'] && $result['data']) {
        $data = $result['data'][0];
        $formSubmitted = ($data['form_submit_flag_aps_3'] == 1); // Check if form was already submitted
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !$formSubmitted) {
    ...
    if (empty($errors)) {
        ...
        if (!$roll) {
            ...
            $active = 1;
            $sql = "UPDATE phd_scholar SET title_of_aps_3 = ?, date_of_aps_3 = ?, dc_comment_in_aps_3 = ?, result_of_aps_3 = ?, form_submit_flag_aps_3 = ? WHERE roll = ?";
