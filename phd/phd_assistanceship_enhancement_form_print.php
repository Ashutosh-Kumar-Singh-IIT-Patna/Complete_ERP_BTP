...
$data = [];
$formName = 'Enhancement of Institute Research Assistantship';  //Important
...
$sql = 'SELECT name_of_scholar, dept, roll, nationality, gender, scholar_phd_category, mobile, email, supervisor, co_supervisor, sponsored_agency_name, proj_num, proj_name, proj_tenure, broad_research_area, dc_comment_in_enhancement, current_stipend_amount, enhanced_stipend_amount, form_submit_flag_assistantship_enhancement,
...
if ($result['success'] && $result['data']) {
    $data = $result['data'][0];
    $formSubmitted = ($data['form_submit_flag_assistantship_enhancement'] == 1);  //Important
} else {
    die('There is some issue with server. Please Try Again.');
}
if (!$formSubmitted) { 
...
//$titleOfForm = $data['thesis_title'] ?? '';  //Important
$dateOfSeminar = formatDate($data['date_of_enhancement'] ?? '');  //Important 
$commentByDC = $data['dc_comment_in_enhancement'] ?? '';  //Important
$currentStipendAmount = $data['current_stipend_amount'] ?? '';
$enhancedStipendAmount = $data['enhanced_stipend_amount'] ?? '';
...
$duration = calculateDuration($doctoralComitteFormation, $finalRecomendationForDegree);
    ...
    $html = '<b>15. Comment By DC : </b>'.$commentByDC;
    $pdf -> MultiCell($available_width, 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    ...
    $pdf->Ln(10);
    $html = '<b>
In view of the recommendation of the Doctoral Committee, the Scholar be granted the Enhanced Assistantship of Rs. '.$enhancedStipendAmount.' per month from 1st Feb 2023.<br><br><br>
Necessary Office Order may be issued.</b>';
    $pdf -> MultiCell($available_width , 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '1. The candidate be permitted to defend the thesis in an open viva-voce examination.';
    $pdf -> MultiCell($available_width , 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = '2.	<b>'.$externalExaminer.'</b>   be invited, to be a member of the Viva-Voce Board.';
    $pdf -> MultiCell($available_width , 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
