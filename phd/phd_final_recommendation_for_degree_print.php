...
$sql = 'SELECT name_of_scholar, dept, roll, nationality, gender, scholar_phd_category, mobile, email, supervisor, co_supervisor, sponsored_agency_name, proj_num, proj_name, proj_tenure, broad_research_area, thesis_title, form_submit_flag_final_recommendation, 
...
if ($result['success'] && $result['data']) {
    $data = $result['data'][0];
    $formSubmitted = ($data['form_submit_flag_final_recommendation'] == 1);
} else {
...
$dateOfSeminar = formatDate($data['date_of_final_recommendation'] ?? '');
// $commentByDC = "Good";
...
$duration = calculateDuration($doctoralComitteFormation, $finalRecomendationForDegree);
    ...
    // $pdf -> MultiCell($available_width, 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $pdf->Ln(10);
    $html = '<b>Recommendation of the Viva-Voce Board</b>';
    $pdf -> MultiCell($available_width , 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
    $html = 'The performance of the candidate was Excellent / Very Good / Good / Satisfactory (Tick one) and recommends the award of the degree of Ph.D. to the candidate';
    $pdf -> MultiCell($available_width , 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
