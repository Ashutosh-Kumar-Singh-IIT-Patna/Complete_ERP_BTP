...
$data = [];
$formName = 'Synopsis Submission - 2';  //Important
...
class MYPDF extends TCPDF {
    ...
    public function Footer() {
        ...
        $available_width = $this->getPageWidth() - $this->lMargin - $this->rMargin;
        $html = 'NOTE: This certificate is required to be sent to the Academic.';
        $this->MultiCell($available_width, 0, $html, 0, 'C', false, 1, '', '', true);
    }   
}

$sql = 'SELECT name_of_scholar, dept, roll, nationality, gender, scholar_phd_category, mobile, email, supervisor, co_supervisor, sponsored_agency_name, proj_num, proj_name, proj_tenure, broad_research_area, title_of_synopsis_attempt_2, result_of_synopsis_attempt_2, dc_comment_in_synopsis_attempt_2, form_submit_flag_synopsis_attempt_2,
...
if ($result['success'] && $result['data']) {
    $data = $result['data'][0];
    $formSubmitted = ($data['form_submit_flag_synopsis_attempt_2'] == 1);  //Important
} else {
...
$titleOfForm = $data['title_of_synopsis_attempt_2'] ?? '';  //Important
$dateOfSeminar = formatDate($data['date_of_synopsis_attempt_2'] ?? '');  //Important
$commentByDC = $data['dc_comment_in_synopsis_attempt_2'] ?? '';  //Important
$performance = $data['result_of_synopsis_attempt_2'];  // Important
...
$duration = calculateDuration($doctoralComitteFormation, $finalRecomendationForDegree);
    ...
    $html = '<b>NOTE: This certificate is required to be sent to the Academic</b>';
    $pdf -> MultiCell($available_width, 0,   $html,   1,   'L',   false,   1,    '',    '',  true,  0,   true,  true,   0,   'T', false);
