...
$data = [];
$formName = 'PhD Enrollment Program';
...
if ($roll) {
    ...
    $sql = "SELECT name_of_scholar, dept, roll, nationality, gender, category, scholar_phd_category, mobile, email, proj_num, proj_name, proj_tenure, sponsored_agency_name, 
            form_submit_flag_enrollment, dc_chair, supervisor, co_supervisor, dc_internal_member, dc_external_member, dc_additional_member_1
    ...
    if ($result['success'] && $result['data']) {
        $data = $result['data'][0];
        $formSubmitted = ($data['form_submit_flag_enrollment'] == 1); // Check if form was already submitted
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $updateData = [];
    if (!empty($_POST['date_of_enrollment'])) {
        $updateData['date_of_enrollment'] = clean_input($_POST['date_of_enrollment']);
    } else {
        $errors['date_of_enrollment'] = "Date of Comprehensive Exam Committee is required";
    }

    if (!empty($updateData)) {
        $updateData['form_submit_flag_enrollment'] = 1;
        ...
        <div class="card shadow p-4">
            ...
            <?php else: ?>
                <form action="" method="POST">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold" for="date_of_enrollment">Date of <?php echo $formName ?>:</label>
                        <input type="text" id="date_of_enrollment" name="date_of_enrollment" class="form-control" placeholder="YYYY-MM-DD" required>
                        <span class="error"><?php echo $errors['date_of_enrollment'] ?? ''; ?></span>
                    </div>
